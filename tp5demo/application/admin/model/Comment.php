...
class Comment extends BaseModel
{
    ...
    {
        $params = array();
        $sql = "select a.id from s_news_comment as a ,s_news as b where a.id=b.news_id where 1" ;
        $sql_count = "select count(1) from s_news_comment as a ,s_news as b where a.id=b.news_id where 1" ;
        ...
        if($str_ids)
        {
            $sql = "select a.*,b.title as news_title from s_news_comment as a ,s_news as b where a.id=b.news_id where a.id in (".$str_ids.")" ;
            $sql .= $sql_order;
            $list = Db::query($sql);
        }
        if($list)
